...
static GVariant *
get_result_metas (const gchar * const *page_ids,
...
static GVariant *
get_search_results (const gchar * const *terms,
                    GHashTable          *page_data)
{
    GVariantBuilder results;
    gchar *term = g_strjoinv (" ", (gchar **) terms);
    GHashTableIter iter;
    const gchar *page_id;
    PageData *data;
...
typedef GVariant * (*ResultGetter) (const gchar * const *terms,
                                    GHashTable          *page_data);
                           ...
                           ResultGetter            result_getter)
{
    ...
    delayed->terms = g_strdupv ((gchar **) terms);
...
static void
yelp_launch (const gchar           *string_parameter,
             YelpSearchProviderApp *app,
             GDBusMethodInvocation *invocation,
             const gchar           *action)
{
...
{
    YelpSearchProviderApp *app = YELP_SEARCH_PROVIDER_APP (user_data);
    gchar *joined_terms = g_strjoinv (" ", (gchar **) terms);
                 ...
                 const GError          *error)
{
    ...
    for (i = 0; i < self->delayed_result_getters->len; i++) {
        ...
        g_dbus_method_invocation_return_value (delayed->invocation,
                                               delayed->result_getter ((const gchar * const *) delayed->terms,
